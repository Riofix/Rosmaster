 ...
 * ======================================================= */
MPU_RawData_t g_mpu_raw = {0};
MPU_Attitude_t g_mpu_attitude = {0.0f, 0.0f, 0.0f};
MPU_Quaternion_t g_mpu_quat = {1.0f, 0.0f, 0.0f, 0.0f};

/* Mahony 算法参数 */
#define MAHONY_Kp 2.0f                                             // 比例增益
#define MAHONY_Ki 0.005f                                           // 积分增益
static float s_q_exInt = 0.0f, s_q_eyInt = 0.0f, s_q_ezInt = 0.0f; // 积分累计误差
...
/**
 * @brief  四元数姿态解算 (Mahony 算法)
 * @param  raw  原始数据
 * @param  dt   调用频率 (s)
 */
void MPU_QuaternionUpdate(MPU_RawData_t *raw, float dt)
{
    float q0 = g_mpu_quat.q0;
    float q1 = g_mpu_quat.q1;
    float q2 = g_mpu_quat.q2;
    float q3 = g_mpu_quat.q3;
    ...
    float gx = ((float)raw->gx - s_gyro_offset[0]) / GYRO_SCALE * (3.14159265f / 180.0f); // 换算到 rad/s
    float gy = ((float)raw->gy - s_gyro_offset[1]) / GYRO_SCALE * (3.14159265f / 180.0f);
    float gz = ((float)raw->gz - s_gyro_offset[2]) / GYRO_SCALE * (3.14159265f / 180.0f);
    ...
    if (ax != 0.0f || ay != 0.0f || az != 0.0f)
    {
        ...
        /* 积分误差 */
        s_q_exInt += ex * MAHONY_Ki;
        s_q_eyInt += ey * MAHONY_Ki;
        s_q_ezInt += ez * MAHONY_Ki;

        /* PI 补偿 */
        gx += MAHONY_Kp * ex + s_q_exInt;
        gy += MAHONY_Kp * ey + s_q_eyInt;
        gz += MAHONY_Kp * ez + s_q_ezInt;
    }
    ...
    g_mpu_quat.q0 = q0 / norm;
    g_mpu_quat.q1 = q1 / norm;
    g_mpu_quat.q2 = q2 / norm;
    g_mpu_quat.q3 = q3 / norm;

    /* 转换到欧拉角 (角度制) */
    g_mpu_attitude.roll = atan2f(2.0f * (g_mpu_quat.q0 * g_mpu_quat.q1 + g_mpu_quat.q2 * g_mpu_quat.q3), 1.0f - 2.0f * (g_mpu_quat.q1 * g_mpu_quat.q1 + g_mpu_quat.q2 * g_mpu_quat.q2)) * (180.0f / 3.14159265f);
    g_mpu_attitude.pitch = asinf(2.0f * (g_mpu_quat.q0 * g_mpu_quat.q2 - g_mpu_quat.q3 * g_mpu_quat.q1)) * (180.0f / 3.14159265f);
    g_mpu_attitude.yaw = atan2f(2.0f * (g_mpu_quat.q0 * g_mpu_quat.q3 + g_mpu_quat.q1 * g_mpu_quat.q2), 1.0f - 2.0f * (g_mpu_quat.q2 * g_mpu_quat.q2 + g_mpu_quat.q3 * g_mpu_quat.q3)) * (180.0f / 3.14159265f);
}
